...
def find_distance(mom: cyvcf2.Variant, dad: cyvcf2.Variant, kid: cyvcf2.Variant, pow: int) -> ty.Tuple[int, ty.List[int]]:
    mom_mc = mom.format("MC")[0]
    dad_mc = dad.format("MC")[0]
    kid_mc = dad.format("MC")[0]
...
def vmc_fmt(variant: cyvcf2.Variant) -> str:
    ...
    """
    return f'{",".join(str(x) for x in variant.format("MC")[0])}'
...
def main(mom_vcf: pathlib.Path, dad_vcf: pathlib.Path, kid_vcfs: ty.List[pathlib.Path], *, pow: int = 1):
    ...
    for vs in zip(*vcfs):
        mom, dad = vs[:2]
        assert mom.POS == dad.POS and mom.CHROM == dad.CHROM and mom.REF == dad.REF and mom.ALT == dad.ALT
        for i, kid in enumerate(vs[2:]):
            assert mom.POS == kid.POS and mom.CHROM == kid.CHROM and mom.REF == kid.REF and mom.ALT == kid.ALT
            d, parental_ht = find_distance(mom, dad, kid, pow)
            print(f'{mom.CHROM}:{mom.POS}:{mom.REF}:{mom.ALT[0]}\t{kid_ids[i]}\t{vmc_fmt(mom)}\t{vmc_fmt(dad)}\t{vmc_fmt(kid)}\t{d}\t{mc_fmt(parental_ht)}')
